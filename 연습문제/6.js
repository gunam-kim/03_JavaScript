...
*/
        ...
        // menuContent 객체에 저장된 input 요소를 모두 .menu(li)의 자식으로 추가
...
addMenu.addEventListener("click", ()=>{
    ...
    // 4-1) li 요소 생성 (+ class명 : menu)
    const li = newEl("li", {}, ["menu"]);

    // 4-2) li에 들어갈 input 3개 만들어서 객체로 반환받기
    const menuContent = createMenuContent();

    // 4-3) li에 menuContent 값을 모두 추가
    for(let key in menuContent){
        li.append(menuContent[key]);
    }

    // 4-4) li를 menuContainer(메뉴판)에 추가
...
deleteMenu.addEventListener("click", ()=>{
    // 5-1) .menu-check(체크박스) 중 체크된 요소만 얻어오기
