/* 
    ...
    쉽게 할 수 있게 해주는 방법
        문자열 전체를 ``(백틱)으로 감싸고 변수/값이 들어가야되는
        부분에는 ${변수명/값} 형태로 작성
            ex)
            const num = 10;
...
function check1(){
    ...
    // number, string 동등 비교
    console.log("num1 === str1 : ", num1 === str1);   // false (자료형이 달라서)
...
function oddEvenCheck(){
    ...
    // input1에 작성된 값 얻어와 number타입으로 변환해 변수 저장
...
function orCheck(){
    // 4는 10을 초과하거나 짝수인가?
    const bool4 = (4 > 10) || (4 % 2 === 0);    // true
    console.log(`결과 1 : ${bool4}`);
    
    // 나이가 18세 미만이거나, 이름이 "홍길동"인가?
    const age = 20;
    const name = "홍길동";
    const bool5 = (age < 18) || (name === "홍길동");    // true
    console.log(`결과 2 : ${bool5}`);

    // 국어 또는 영어 점수가 40점 미만이거나,
    // 평균이 60점 미만인가?
    const kor = 50;
    const eng = 75;
    const bool6 = ((kor < 40) || (eng < 40)) || ((kor + eng) / 2 < 60); // false
    console.log(`결과 3 : ${bool6}`);
}