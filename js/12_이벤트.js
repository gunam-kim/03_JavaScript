...
/*
전달 인자(argument)
    : 함수 호출 시 ()안에 작성하는 값. 함수 매개변수로 전달됨
매개 변수(parameter)
    : 함수 정의 시 ()에 작성하는 변수. 호출 시 전달된 값을 저장
*/
function check1(div){
    // 매개 변수 div : 클릭된 div요소
    // console.log(div);

// 클릭되면 클릭된 숫자를 버튼(div)에 출력
    // 1) 버튼에 작성된 내용 얻어오기
...
/* 고전 이벤트 모델 확인 */
    ...
    testA.onclick = function(){
        // testA 요소가 가지고 있는 onclick변수에   -> 이벤트 리스너
...
test1.addEventListener("click", function(){
        ...
        let current = test1.style.opacity;  // opacity : 불투명 정도를 설정
            // opacity = 0;     투명
            // opacity = 1;     불투명
...
for(let i = 0; i < btns.length; i++){
    ...
    // 각 버튼에 클릭되었을 때(이벤트 리스너)의 동작(이벤트 핸들러)을 추가
...
/* 실습 문제 */
// 변경하기 버튼 클릭 시 input요소에 작성된 색상값을 얻어와
...
const displays = document.querySelectorAll(".display");     // 출력화면 5개
    // 위아래로 연결된 .display와 .inputColor의 인덱스 번호가 같다
